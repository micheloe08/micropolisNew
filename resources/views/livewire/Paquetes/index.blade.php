<div>
    @if($updateMode)
        @include('livewire.Paquetes.update')
    @else
        @include('livewire.Paquetes.create')
    @endif
        ...
        <tbody>
            ...
            <tr class="odd:bg-white odd:dark:bg-gray-900 even:bg-gray-50 even:dark:bg-gray-800 border-b dark:border-gray-700">
                <th scope="row" class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">{{ $item->id }} </th>
                    ...
                    <td scope="col">{{ $item->sucursales->nombre}}</td>
                    <td scope="col">${{ number_format($item->precio_base, 2) }}</td>
                    <td scope="col">
